...
use agent_click_core::platform::{AppInfo, WindowInfo};
use agent_click_core::selector::Selector;
use agent_click_core::{Error, Platform};
    ...
    async fn get_connection(&self, app: &str) -> Option<std::sync::Arc<CdpConnection>> {
        ...
        let port = self
            ...
            .or_else(|| {
                ...
                eprintln!("agent-click: relaunching {app} with CDP support...");
...
impl<P: Platform> Platform for ElectronAwarePlatform<P> {
    ...
    async fn find(&self, selector: &Selector) -> agent_click_core::Result<Vec<AccessibilityNode>> {
    ...
    async fn perform(&self, action: &Action) -> agent_click_core::Result<ActionResult> {
    ...
    async fn text(&self, app: Option<&str>) -> agent_click_core::Result<String> {
        if let Some(app_name) = app {
            if let Some(conn) = self.get_connection(app_name).await {
    ...
    async fn press(&self, selector: &Selector) -> agent_click_core::Result<bool> {
        if let Some(ref id) = selector.id {
            if let Some(tag) = dom::extract_cdp_tag(id) {
                if let Some(ref app_name) = selector.app {
                    if let Some(conn) = self.get_connection(app_name).await {
                        tracing::debug!("pressing via CDP: data-agent-click={tag}");
    ...
    async fn set_value(&self, selector: &Selector, value: &str) -> agent_click_core::Result<bool> {
        if let Some(ref id) = selector.id {
            if let Some(tag) = dom::extract_cdp_tag(id) {
                if let Some(ref app_name) = selector.app {
                    if let Some(conn) = self.get_connection(app_name).await {
                        tracing::debug!("typing via CDP: data-agent-click={tag}");
    ...
    async fn activate(&self, app: &str) -> agent_click_core::Result<()> {
    ...
    async fn focused(&self) -> agent_click_core::Result<AccessibilityNode> {
    ...
    async fn windows(&self, app: Option<&str>) -> agent_click_core::Result<Vec<WindowInfo>> {
        self.inner.windows(app).await
    }

    async fn check_permissions(&self) -> agent_click_core::Result<bool> {
        self.inner.check_permissions().await
    }

    async fn scroll_to_visible(&self, selector: &Selector) -> agent_click_core::Result<bool> {
        self.inner.scroll_to_visible(selector).await
    }

    async fn open_application(&self, app: &str) -> agent_click_core::Result<()> {
        self.inner.open_application(app).await
    }

    async fn move_window(&self, app: &str, x: f64, y: f64) -> agent_click_core::Result<bool> {
