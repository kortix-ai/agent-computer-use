...
(function() {
    ...
    function walkNode(el, depth, maxDepth) {
        ...
        if (interactive) {
            acId = String(counter++);
            el.setAttribute('data-agent-click', acId);
        }
...
pub async fn click_by_tag(conn: &CdpConnection, tag: &str) -> agent_click_core::Result<bool> {
    let js = format!(
        r#"(function() {{
            const el = document.querySelector('[data-agent-click="{tag}"]');
            if (!el) return false;
...
pub async fn click_by_css(conn: &CdpConnection, css: &str) -> agent_click_core::Result<bool> {
...
pub async fn dispatch_key(conn: &CdpConnection, key_expr: &str) -> agent_click_core::Result<()> {
...
pub async fn insert_text(conn: &CdpConnection, text: &str) -> agent_click_core::Result<()> {
    conn.send(
        "Input.insertText",
...
pub async fn get_page_text(conn: &CdpConnection) -> agent_click_core::Result<String> {
