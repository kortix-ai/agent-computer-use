...
<PageHeader
  title="snapshot"
  description="Dump the accessibility tree of any application and get refs you can use with every other command."
/>

## Usage

```bash
agent-cu snapshot [OPTIONS]
```
...
```bash
agent-cu snapshot -a Calculator -i -c        # interactive + compact (best for AI)
agent-cu snapshot -a Safari -i -c -d 8       # deeper tree
agent-cu snapshot -a Music                   # full tree (verbose)
```
...
```bash
agent-cu click @e3          # clicks "7"
agent-cu get-value @e1      # reads "Delete" button info
```
...
- Refs are cached in `~/.agent-cu/refs.json` with a 5-minute staleness warning
