...
When you run `agent-cu click @e5`, agent-cu sends an `AXPress` action to the element via the accessibility API. This is the same mechanism VoiceOver uses — it doesn't move the mouse cursor or activate the window. The app processes the press in its event loop without becoming frontmost.

If AXPress isn't supported for a particular element (rare), agent-cu falls back to coordinate-based mouse clicking, which does require the window to be visible.
