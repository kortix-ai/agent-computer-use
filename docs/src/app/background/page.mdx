...
When you run `agent-click click @e5`, agent-click sends an `AXPress` action to the element via the accessibility API. This is the same mechanism VoiceOver uses — it doesn't move the mouse cursor or activate the window. The app processes the press in its event loop without becoming frontmost.
